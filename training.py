...
def load_data():
    ...
    digits_data_train = pd.read_csv("mnist_train.csv")
    digits_data_test = pd.read_csv("mnist_test.csv")
...
history = model.fit(generator.flow(x_train, y_train, batch_size=64), epochs=200, steps_per_epoch=x_train.shape[0]//64, validation_data=(x_test, y_test))
